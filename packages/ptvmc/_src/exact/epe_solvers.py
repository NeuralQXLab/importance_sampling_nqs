...
from ptvmc._src.exact.epe_integrator import _split_hamiltonian

OperatorT = AbstractOperator | np.ndarray
ApplyFunT = Callable[[np.ndarray], np.ndarray]


def LPE1(H: OperatorT, dt: float) -> ApplyFunT:
...
def LPE2(H: OperatorT, dt: float) -> ApplyFunT:
    B1 = (1 - 1j) / 2
    B2 = (1 + 1j) / 2
...
def LPE3(H: OperatorT, dt: float) -> ApplyFunT:
...
def LPE4(H: OperatorT, dt: float) -> ApplyFunT:
...
def PPE2(H: OperatorT, dt: float, neumann_order=15) -> ApplyFunT:
    B1 = 1 / 2
    B2 = -1 / 2
...
def PPE4(H: OperatorT, dt: float, neumann_order=15) -> ApplyFunT:
...
def SLPE1(H: OperatorT, dt: float) -> ApplyFunT:
    A1 = 1
    B1 = 1

    Hd, Ho = _split_hamiltonian(H)
    x = -1j * Ho
    z = -1j * Hd
...
def SLPE2(H: OperatorT, dt: float) -> ApplyFunT:
    A1 = (1 - 1j) / 2
    A2 = (1 + 1j) / 2
    B1 = (1 - 1j) / 2
    B2 = (1 + 1j) / 2
...
def SLPE3(H: OperatorT, dt: float) -> ApplyFunT:
...
def SPPE2(H: OperatorT, dt: float, neumann_order=15) -> ApplyFunT:
    A1 = 1 / 2
    A2 = 1 / 2
    B1 = 1 / 2
    B2 = -1 / 2
...
def SPPE3(H: OperatorT, dt: float, neumann_order=15) -> ApplyFunT:
