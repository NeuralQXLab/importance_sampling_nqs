...
def compute_snr_callback(
    ...
    def cb_func(
        ...
        if step % save_every == 0:
            ...
            def compute_snr(q):
                ...
                v = (
                    jnp.sum(
                        ...
                        * (unnorm_pdf(2.0) / q) ** 2
                        * jnp.abs(
                            loc_grad_v - jnp.sum(pdf * loc_grad_v, axis=1)[:, None]
                        )
                        ** 2,
                        axis=1,
                    )
                ...
                return jnp.mean(
                    jnp.abs(jnp.sum(pdf * loc_grad_v, axis=1)) / jnp.sqrt(v)
                )
            ...
            snr_grad = compute_snr(
                jnp.mean(unnorm_pdf(2.0) * jnp.abs(loc_grad_v), axis=0)
            )
