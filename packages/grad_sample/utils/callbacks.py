...
def save_exact_err(fs_state, e_gs, save_every=1, output_dir=None):
    def cb_func(step, logdata, driver, fs_state, e_gs, save_every=1, output_dir=None):
        if driver.step_count % save_every == 0:
            ...
            #     plt.clf()
        return True
    return partial(cb_func, fs_state=fs_state, e_gs=e_gs, save_every=save_every, output_dir=output_dir)

def save_mc_large_err(e_gs, n_s = 2**15, n_sites=None, save_every=1, output_dir=None):
    def cb_func(step, logdata, driver, e_ref, n_s = 2**15, n_sites=None, save_every=1, output_dir=None):
        ...
        return True
    return partial(cb_func, e_ref=e_gs, n_s =n_s, n_sites=n_sites, save_every=save_every, output_dir=output_dir)
...
def compute_snr_callback(fs_state:FullSumState, H_sp, save_every=10, chunk_size_jac=200):
    def cb_func(step, logdata, driver, fs_state:FullSumState, H_sp, save_every=10, chunk_size_jac=200):
        # estimate local grad
        if step % save_every == 0:
    ...
    return partial(cb_func, fs_state=fs_state, H_sp=H_sp, save_every=save_every, chunk_size_jac=chunk_size_jac)
