...
def curved_dist(dp_exact, dp_approx, S):
    ...
    dot = jnp.dot(dp_approx.conj(), S @ dp_exact) * jnp.dot(
        dp_exact.conj(), S @ dp_approx
    )
    norm = jnp.dot(dp_approx.conj(), S @ dp_approx) * jnp.dot(
        dp_exact.conj(), S @ dp_exact
    )
    return (
        jnp.arccos(jnp.sqrt((dot / norm).real)) ** 2
    )  # distance on parameter space with S matrix
