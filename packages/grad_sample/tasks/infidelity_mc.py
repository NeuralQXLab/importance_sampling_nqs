...
class InfidelityMC(Base):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.output_dir = (
            self.base_path
            + f"/Infidelity_opt/L{self.model.graph.n_nodes}/{self.ansatz_name}/{self.alpha}/MC_{self.sample_size}_{self.is_distrib.q_variables['alpha'].item()}/{self.lr}_{self.diag_exp}"
        )
        ...
        self.callbacks = (InvalidLossStopping(),) + tuple(
            smart_instantiate(cb, kwargs_hydra, mode="call") for cb in cfg.callback_list
        )

    def __call__(self):
        self.mc_driver.run(
            n_iter=self.n_iter, out=self.json_log, callback=self.callbacks
        )
