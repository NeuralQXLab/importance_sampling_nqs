...
class FullSumGS(Base):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.output_dir = (
            self.base_path
            + f"/{self.model.name}_{self.model.h}/L{self.model.graph.n_nodes}/{self.ansatz_name}/{self.alpha}/schedule_schedule"
        )
        ...
        if (
            self.chunk_size_jac is not None
            and self.chunk_size_jac < self.model.hilbert_space.n_states
        ):
            self.chunk_size = self.model.hilbert_space.n_states // (
                self.model.hilbert_space.n_states // self.chunk_size_jac
            )
        else:
            self.chunk_size = self.model.hilbert_space.n_states
            ...
            self.vstate = nk.vqs.FullSumState(
