...
class VMC_GS(Base):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.collect_gradient_statistics = self.cfg.get(
            ...
            try:
                with open(self.ckpt, "rb") as f:
                    vars = nk.experimental.vqs.variables_from_file(
                        self.ckpt, self.vstate.variables
                    )
        ...
        if self.is_distrib.name == "overdispersed":
            if self.auto_is:
                self.output_dir = (
                    self.base_path
                    + f"/{self.model.name}_{self.model.h}/L{self.model.graph.n_nodes}/{self.ansatz_name}/{self.alpha}/MC_{self.sample_size}_isauto/{self.lr}_{self.diag_exp}"
                )
            else:
                self.output_dir = (
                    self.base_path
                    + f"/{self.model.name}_{self.model.h}/L{self.model.graph.n_nodes}/{self.ansatz_name}/{self.alpha}/MC_{self.sample_size}_{self.is_distrib.q_variables['alpha'].item()}/{self.lr}_{self.diag_exp}"
                )
        elif self.is_distrib.name == "default":
            self.output_dir = (
                self.base_path
                + f"/{self.model.name}_{self.model.h}/L{self.model.graph.n_nodes}/{self.ansatz_name}/{self.alpha}/MC_{self.sample_size}/{self.lr}_{self.diag_exp}"
            )
        else:
        ...
        self.gs = advd.driver.VMC_NG(
            hamiltonian=self.model.hamiltonian.to_jax_operator(),
            optimizer=self.opt,
        ...
        self.kwargs_hydra["fs_state"] = FullSumState(
            hilbert=self.gs.state.hilbert, model=self.gs.state.model, seed=0
        )
        ...
        if self.save_every != None:
            ...
            self.state_log = nk.logging.StateLog(
                output_prefix=self.state_dir, save_every=self.save_every
            )
            self.out_log = (self.json_log, self.state_log)
        else:
            self.out_log = (self.json_log,)

        self.callbacks = (InvalidLossStopping(),) + tuple(
            smart_instantiate(cb, self.kwargs_hydra, mode="call")
    ...
    def __call__(self):
        ...
        try:
            plt.title(
                f"Relative error w.r.t. exact GS during training, {self.Nsample} samples"
            )
            e_r_fs = data["rel_err"]
