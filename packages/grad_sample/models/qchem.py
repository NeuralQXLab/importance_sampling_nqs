    ...
    def __init__(self, cid=947, use_nat_orb=False):
        ...
        if self.use_nat_orb:
            ha_pyscf = nkx.operator.from_pyscf_molecule(
                mol, mo_coeff=nat_orbs[1]
            ).to_jax_operator()
        else:
            ha_pyscf = nkx.operator.from_pyscf_molecule(mol).to_jax_operator()

        self.hamiltonian = ParticleNumberConservingFermioperator2ndSpinJax.from_fermiop(
            ha_pyscf
        )
        ...
        # make sampling graph
        E_nuc, Tij, Vijkl = nkx.operator.pyscf.TV_from_pyscf_molecule(
