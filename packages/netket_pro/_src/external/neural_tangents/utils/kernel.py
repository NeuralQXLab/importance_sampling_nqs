...
class Kernel:
    ...
    def reverse(self) -> "Kernel":
        ...
        return self.replace(
            cov1=cov1, nngp=nngp, cov2=cov2, ntk=ntk, is_reversed=not self.is_reversed
        )
    ...
    def transpose(self, axes: Optional[Sequence[int]] = None) -> "Kernel":
        ...
        def permute(
            mat: Optional[jnp.ndarray], batch_ndim: int
            ...
            if mat is not None:
                ...
                if not self.diagonal_spatial:
                    _axes = tuple(
                        j
                        for a in _axes
                        for j in (2 * a - batch_ndim, 2 * a - batch_ndim + 1)
                    )
    ...
    def mask(
        self, mask1: Optional[jnp.ndarray], mask2: Optional[jnp.ndarray]
    ) -> "Kernel":
    ...
    def _get_mask_prods(
        ...
        def get_mask_prod(m1, m2, batch_ndim):
            ...
            def reshape(m):
                if m is not None:
                    ...
                    m = jnp.squeeze(
                        jnp.moveaxis(m, (self.batch_axis, self.channel_axis), (0, -1)),
                        -1,
                    )
                    if self.is_reversed:
        ...
        mask22 = (
            get_mask_prod(mask2, mask2, 1 if self.diagonal_batch else 2)
    ...
    def dot_general(
        ...
        def to_kernel_dim(i: int, batch_ndim: int, is_left: bool) -> int:
            if i == self.batch_axis:
                ...
                raise ValueError(
                    f"Batch or contracting dimension {i} cannot be equal "
                    f"to `channel_axis`."
                )
            else:
        ...
        def get_other_dims(batch_ndim: int, is_left: bool) -> list[int]:
            dims = [
                -i - 1 - (0 if is_left or self.diagonal_spatial else n_other)
        ...
        def get_other_non_c_dims() -> list[int]:
            ...
            if not self.diagonal_spatial:
                other_non_c_dims = list(
                    utils.zip_flat(
                        other_non_c_dims,
                        [
                            -i - 1 - n_other
                            for i in range(n_other)
                            if i not in other_dims
                        ],
                    )
                )
        ...
        def dot(
            ...
            return jnp.einsum(
                *operands, get_out_dims(batch_ndim), optimize=True
        ...
        return self.replace(
            ...
            ntk=ntk,
            is_reversed=False,
            batch_axis=utils.axis_after_dot(
                self.batch_axis, input_cs, input_bs, lhs_ndim
            ),
            channel_axis=utils.axis_after_dot(
                self.channel_axis, input_cs, input_bs, lhs_ndim
            ),
        )
    ...
    def __getitem__(self, idx: utils.SliceType) -> "Kernel":
        ...
        spatial_idx = tuple(
            ...
            if i not in (self.batch_axis, self.channel_axis)
        )
        ...
        ntk = (
            self.ntk
            if (
                self.ntk is None or self.ntk.ndim == 0
            )  # pytype: disable=attribute-error
            else self.ntk[nngp_slice]
        )
