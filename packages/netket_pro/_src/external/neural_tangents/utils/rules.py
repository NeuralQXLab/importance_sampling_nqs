...
class Structure:
    ...
    def __and__(self, other):
        ...
        out_trace = tuple(
            self.out_trace[i]
            for i in range(len(self.out_trace))
            if self.in_trace[i] in other.in_trace
        )

        assert len(in_trace) == len(out_trace), (self, other)

        out_diagonal = tuple(i for i in self.out_diagonal if i in other.out_diagonal)
        in_diagonal = tuple(
            i
            for ix, i in enumerate(self.in_diagonal)
            if self.out_diagonal[ix] in other.out_diagonal
        )

        out_broadcast = tuple(i for i in self.out_broadcast if i in other.out_broadcast)

        in_broadcast = tuple(i for i in self.out_broadcast if i in other.out_broadcast)
...
def get_structure_cache(jaxpr: Jaxpr, _s_rules: bool) -> dict[Var, Structure]:
...
def get_id_structure(inval: AbstractValue, _s_rules: bool) -> Structure:
...
def _eye_like(out_shaped: ShapedArray, in_shaped: ShapedArray) -> jnp.ndarray:
    ...
    eye = eye.reshape(
        out_shaped.shape + in_shaped.shape
...
def _dot_general_s(
    eqn: JaxprEqn, idx: int, invals: list[ShapedArray], cts_in: ShapedArray
) -> Structure:
    ...
    in_trace = tuple(
        i for i in range(self.ndim) if (i not in self_c_dims) and (i not in self_b_dims)
    )
    out_trace = tuple(
        utils.axis_after_dot(
            i, self_c_dims, self_b_dims, lhs_ndim=None if idx == 0 else other.ndim
        )
        for i in in_trace
    )
...
def _dot_general_j(
    ...
    self_ncb_dims = tuple(
        i for i in range(self.ndim) if i not in self_c_dims + self_b_dims
    )
    self_nc_dims = tuple(i for i in range(self.ndim) if i not in self_c_dims)

    j = jnp.moveaxis(  # pytype: disable=wrong-arg-types  # jnp-type
        other,
        other_b_dims + tuple(d[1] for d in sorted(zip(self_c_dims, other_c_dims))),
        tuple(range(len(other_b_dims))) + tuple(range(-len(other_c_dims), 0)),
    )

    self_ncb_out = tuple(
        utils.axis_after_dot(
            i, self_c_dims, self_b_dims, other.ndim if idx == 1 else None
        )
...
def _conv_general_dilated_s(
    ...
    if rhs.shape[rhs_spec[0]] == feature_group_count and rhs.shape[rhs_spec[1]] == 1:
        ...
        lhs.shape[lhs_spec[0]] == batch_group_count
        and rhs.shape[rhs_spec[0]] == batch_group_count
    ):
        return Structure(
            in_trace=(),
            in_trace_idxs=(),
...
def _conv_general_dilated_j(
    ...
    if n_groups_b > 1:
        ...
        j = j.reshape(
            j.shape[:-2] + (n_channels_in, *filter_shape, n_groups_b, batch_size_in)
        )
    ...
    eye = jnp.expand_dims(
        eye, [i for i in range(j.ndim) if i not in (out_spec[1], ndim + rhs_spec[0])]
    )
    j = jnp.kron(j, eye)
    return j
...
def _conv_general_dilated_e(
    ...
    if (
        params["feature_group_count"] > lhs.shape[dn[0][1]]
        or params["feature_group_count"] > rhs.shape[dn[1][0]]
    ):
        params["feature_group_count"] = 1

    if (
        params["batch_group_count"] > rhs.shape[dn[1][0]]
        or params["batch_group_count"] > lhs.shape[dn[0][0]]
    ):
...
def _add_j(
    ...
    j = jnp.broadcast_to(
        j, cts_in.shape + invals[idx].shape
    )  # pytype: disable=unsupported-operands  # always-use-return-annotations
    if is_sub and idx == 1:
        j = -j
    return j
...
def _mul_j(
    ...
    if inval.size == 0:
        return jnp.zeros(
            cts_in.shape + inval.shape, inval.dtype
        )  # pytype: disable=unsupported-operands  # always-use-return-annotations
...
def _rev_s(
    ...
    in_trace = out_trace = tuple(
        i for i in range(invals[idx].ndim) if i not in dimensions
    )
...
def _broadcast_in_dim_s(
    ...
    out_broadcast = tuple(
        i for i in range(cts_in.ndim) if i not in broadcast_dimensions
    )
...
def _broadcast_in_dim_j(
    ...
    j = lax.broadcast_in_dim(
        j,
        cts_in.shape
        + inval.shape,  # pytype: disable=unsupported-operands  # always-use-return-annotations
        broadcast_dimensions=eqn.params["broadcast_dimensions"]
...
def _reduce_window_sum_s(
    ...
    for i in range(cts_in.ndim):
        if (
            eqn.params["base_dilation"][i] == 1
            and eqn.params["padding"][i] == (0, 0)
            and eqn.params["window_dilation"][i] == 1
            and eqn.params["window_dimensions"][i] == 1
            and eqn.params["window_strides"][i] == 1
        ):
            out_trace += (i,)
...
def _pad_s(
    ...
    out_trace = tuple(i for i in range(cts_in.ndim) if padding_config[i] == (0, 0, 0))
    in_trace = out_trace
...
def _eye_s(
    eqn: Optional[JaxprEqn], idx: int, invals: list[ShapedArray], cts_in: ShapedArray
) -> Structure:
...
def _zeros_like_j(
    ...
    return jnp.zeros(
        cts_in.shape + invals[idx].shape, cts_in.dtype
    )  # pytype: disable=unsupported-operands  # always-use-return-annotations
...
def _squeeze_s(
    ...
    out_trace = tuple(range(cts_in.ndim))
    in_trace = tuple(
        i for i in range(invals[idx].ndim) if i not in eqn.params["dimensions"]
    )
    return Structure(
        out_trace=out_trace,
        in_trace=in_trace,
