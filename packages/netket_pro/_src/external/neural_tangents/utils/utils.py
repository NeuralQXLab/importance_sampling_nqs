    ...
    def getter_decorator(fn):
        ...
        def getter_fn(*args, **kwargs):
            ...
            def canonicalize_output(out):
                ...
                if isinstance(out, types.GeneratorType):
                    return (
                        ReturnType(*tuple(output[g] for g in get)) for output in out
                    )
                else:
...
def reverse_zipped(x: _ArrayOrShape, start_axis: int = 0) -> _ArrayOrShape:
    if x is not None:
        ndim = _get_ndim(x)
        source_axes = tuple(
            j for i in range(ndim - 2, start_axis - 1, -2) for j in (i, i + 1)
        )
...
def mask(
    x: Optional[jnp.ndarray], mask_mat: Optional[jnp.ndarray]
...
def make_2d(
    x: Optional[jnp.ndarray], start_axis: int = 0, end_axis: Optional[int] = None
) -> Optional[jnp.ndarray]:
...
def canonicalize_idx(idx: SliceType, ndim: int) -> tuple[Union[int, slice], ...]:
