...
def taylor_expand(f: ApplyFn, params: PyTree, degree: int) -> ApplyFn:
    ...
    def f_tayl(p, *args, **kwargs):
        dparams = _sub(p, params)
        return taylorize_r(
            lambda param: f(param, *args, **kwargs), params, dparams, degree, 0
        )
...
def empirical_nngp_fn(
    f: ApplyFn, trace_axes: Axes = (-1,), diagonal_axes: Axes = ()
) -> EmpiricalKernelFn:
...
def _jacobian_contraction_ntk_fn(
    f: ApplyFn, trace_axes: Axes, diagonal_axes: Axes, vmap_axes: VMapAxes, **kwargs
) -> EmpiricalKernelFn:
...
def _ntk_vector_products_ntk_fn(
    ...
    def ntk_fn(
        ...
        if not utils.all_none(x_axis) or not utils.all_none(kw_axes):
            ...
            get_ntk = vmap(
                vmap(get_ntk, in_axes1, fx_axis), in_axes2, _add(fx_axis, _ndim(fx1))
            )

        ntk = get_ntk(x1, x2, *args1, *args2)
        ntk = tree_map(lambda x: _trace_and_diagonal(x, trace_axes, diagonal_axes), ntk)
        return ntk
...
def _structured_derivatives_ntk_fn(
    ...
    def sum_and_contract(
            ...
            for df_dy_1, dy_dw_1_ in zip(df_dys_1, dy_dws_1):
                for df_dy_2, dy_dw_2_ in zip(df_dys_2, dy_dws_2):
                    ...
                    df_dy_dims_1, df_dy_dims_2, out_dims = _get_dims(
                        df_dy_1, df_dy_2, ndim, _trace_axes, _diagonal_axes
                    )
                    ...
                    if len(s1.out_diagonal) != len(s2.out_diagonal):
                        raise NotImplementedError(
                            "Different number of diagonal_axes 1/2."
                        )

                    for i, (id_1, id_2) in enumerate(
                        zip(s1.out_diagonal, s2.out_diagonal)
                    ):
                        # TODO(romann): compute based on array dimensions.
                        axis_shift = (
                            -100_000
                        )  # Huge axis shift to ensure unique axis ids.
                    ...
                    for i in range(ndim, df_dy_1.ndim):
                        if i - ndim not in (
                            s1.out_trace + s1.out_diagonal + s1.out_broadcast
                        ):
                    ...
                    for i in range(ndim, df_dy_2.ndim):
                        if i - ndim not in (
                            s2.out_trace + s2.out_diagonal + s2.out_broadcast
                        ):
        ...
        ntk = tree_reduce(
            ...
            tree_map(
                ...
                dy_dws_2,
                is_leaf=lambda x: (
                    x == [] or (isinstance(x, list) and isinstance(x[0], jnp.ndarray))
                ),
            ),
            jnp.zeros((), dtype),
        )
    ...
    def ntk_fn(
        ...
        def j_fn(x, *args):
            _kwargs = {k: v for k, v in zip(keys, args)}
            fx = _get_f_params(f, x, x_axis, fx_axis, kw_axes, **_kwargs)
            df_dys, dy_dws = _get_df_dys_and_dy_dws(
                fn=fx, params=params, _j_rules=_j_rules, _s_rules=_s_rules, _fwd=_fwd
            )
        ...
        df_dys_2, dy_dws_2 = (
            j_fn(x2, *args2) if not utils.all_none(x2) else (df_dys_1, dy_dws_1)
        )

        fx_axis, dtype = _get_fx_axis_and_dtype(fx1, fx_axis, params)
        ntk = tree_map(
            functools.partial(
                sum_and_contract, dy_dws_1=dy_dws_1, dy_dws_2=dy_dws_2, dtype=dtype
            ),
            fx1,
            fx2,
            fx_axis,
...
def empirical_kernel_fn(
    ...
    Returns:
      ...
      given network `f`.
    """
    kwargs = dict(f=f, trace_axes=trace_axes, diagonal_axes=diagonal_axes)
    ...
    def kernel_fn(
        ...
        **apply_fn_kwargs,
        ...
        out_dict = {g: kernel_fns[g](x1, x2, params, **apply_fn_kwargs) for g in get}
...
def empirical_ntk_vp_fn(
    f: ApplyFn, x1: PyTree, x2: Optional[PyTree], params: PyTree, **apply_fn_kwargs
...
def _trace_and_diagonal(
    ntk: jnp.ndarray, trace_axes: Axes, diagonal_axes: Axes
) -> jnp.ndarray:
...
def _dict_of_tree_to_tree_of_dict(
    out_dict: dict[str, PyTree], get: tuple[str, ...]
) -> PyTree:
    ...
    # a parallel layer.
    return tree_map(
        lambda *x: dict((g, v) for g, v in zip(get, x)), *[out_dict[g] for g in get]
    )
...
def _get_f1_f2(
    ...
    f2 = (
        ...
        else _get_f_params(f, x2, x_axis, fx_axis, kw_axes, **_kwargs2)
    )
    return f1, f2
...
def _check_einsum_no_broadcast(arrays: list[jnp.ndarray], dims: list[list[int]]):
    ...
    for idx_1, (a1, dims_1) in enumerate(zip(arrays, dims)):
        if len(set(dims_1)) != len(dims_1):
            raise ValueError(
                f"Dimensions {idx_1} contain duplicate axes: " f"{dims_1}."
            )
            ...
            for idx_2, (a2, dims_2) in enumerate(zip(arrays, dims)):
                if dim_1 in dims_2:
                    ...
                    if sz_idx_2 != sz_idx_1:
                        raise ValueError(
                            f"Arrays {idx_1} and {idx_2} mismatch "
...
def _vmap(f: Callable, in_axes, out_axes, squeeze_out: bool = True) -> Callable:
    ...
    @utils.wraps(f)
    def f_vmapped(*args):
        args = tree_map(_expand_dims, args, in_axes_plus_1, is_leaf=_is_abstract_array)
...
def _unravel_dfs(dfs: PyTree, params: PyTree, y: PyTree) -> PyTree:
    ...
    if tree_structure(dfs).num_leaves > 0:
        dfs = tree_transpose(
            tree_structure(tree_map(lambda x, y: [x] * len(y), params, dfs)),
            tree_structure(y),
            dfs,
        )
...
def _get_df_dys_and_dy_dws(
    ...
    def primals_out_and_pullback(mode: _MODE) -> PyTree:
        return _get_primals_out_and_pullback(fn, mode, _j_rules, _s_rules, _fwd, params)
...
def _get_primals_out_and_pullback(
    ...
    *primals_in: PyTree,
    ...
    fn_flat, out_tree = jax.api_util.flatten_fun_nokwargs(lu.wrap_init(fn), in_tree)
    ...
    def pullback_fn(*cts_in: PyTree):
        ...
        cts_out = _backward_pass(
            jaxpr,
            mode=mode,
            consts=consts,
            primals_in=dummy_args,
            cotangents_in=cts_in,
            _j_rules=_j_rules,
            _s_rules=_s_rules,
            _fwd=_fwd,
        )
...
def _backward_pass(
    ...
    list[list[Union[jnp.ndarray, Zero]]],
    ...
    with ctx:
        map(
            functools.partial(_write_cotangent, "outvars", ct_env),
            jaxpr.outvars,
            cotangents_in,
        )
            ...
            for i_eqn, eq_invar in enumerate(eqn.invars):
                if eq_invar in jaxpr.invars:
                    ...
                    if mode == _MODE.DF_DY:
                        ...
                        for in_d, out_d in zip(
                            structure.in_diagonal, structure.out_diagonal
                        ):
                            in_axes = [
                                None if isinstance(invals[ix], UndefinedPrimal) else i
...
def _backprop_step(
    ...
    if do_write_cotangents:
        ...
        map(
            functools.partial(_write_cotangent, eqn.primitive, ct_env),
            eqn.invars,
            cts_out,
        )
...
def _trim_cotangents(cts_in: ShapedArray, structure: rules.Structure) -> ShapedArray:
    cts_in = _trim_axis(
        cts_in, structure.out_trace + structure.out_broadcast + structure.out_diagonal
    )
    cts_in: ShapedArray
    return cts_in
...
def _trim_invals(
    ...
    for i in range(len(trimmed_invals)):
        for in_d in sorted(
            [axis[i] for axis in structure.in_diagonal if axis[i] is not None],
            reverse=True,
        ):
...
def _trim_eqn(
    ...
    if eqn.primitive in rules.EQN_PARAMS_RULES:
        ...
        trimmed_invals_e = [
            i.aval if isinstance(i, UndefinedPrimal) else i for i in trimmed_invals
        ]
...
def _trim_axis(
    ...
    if isinstance(x, (ShapedArray, jnp.ndarray)):
        return ShapedArray(
            [1 if i in axis else x.shape[i] for i in range(x.ndim)], dtype=x.dtype
        )
...
def _eqn_jvp_fn(
    eqn: Optional[JaxprEqn], idx: int, tangents: jnp.ndarray, *invals
) -> jnp.ndarray:
...
def _eqn_vjp_fn(
    eqn: Optional[JaxprEqn], cts_in: jnp.ndarray, *invals
) -> tuple[jnp.ndarray, ...]:
...
def _get_jacobian(
    ...
    dy_dw_shape_ = (
        dy_dw.aval.shape if isinstance(dy_dw, Zero) else dy_dw.shape
...
def _write_cotangent(
    ...
    if jax.config.jax_enable_checks:
        ...
        joined_aval = (
            core.lattice_join(v.aval, ct_aval).strip_weak_type().strip_named_shape()
        )
...
def _write_primal(
    env: dict[Var, jnp.ndarray], v: Var, val: Union[jnp.ndarray, UndefinedPrimal]
):
...
def _get_fwd(
    _fwd: Optional[bool], cts_in_shape: tuple[int, ...], inval_shape: tuple[int, ...]
) -> bool:
    if _fwd is None:
...
def _unravel_array_into_pytree(pytree: PyTree, axis: int, arr: jnp.ndarray) -> PyTree:
    """Similar to `jax.api._unravel_array_into_pytree` without host-side ops."""
    leaves, treedef = tree_flatten(pytree)
    if arr.ndim > 0:
        axis %= arr.ndim
    shapes = [arr.shape[:axis] + jnp.shape(l) + arr.shape[axis + 1 :] for l in leaves]
